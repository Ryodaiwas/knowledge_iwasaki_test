...
provider "aws" {
  region  = "ap-northeast-1" # region of the user account
  profile = "default"
}
...
#       "image": "${aws_ecr_repository.knowledgebase-test-ecr-repo.repository_url}",
#       "essential": true,
#       "portMappings": [
#         {
#           "containerPort": 3000,
...
#     subnets          = ["${aws_default_subnet.default_subnet_a.id}", "${aws_default_subnet.default_subnet_d.id}", "${aws_default_subnet.default_subnet_c.id}"]
